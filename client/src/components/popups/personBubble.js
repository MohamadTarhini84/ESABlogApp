...
function Bubble(props){
    ...
    let side=user.data.id===props.chat.id?"self-end flex-row-reverse":"";
    let colour=user.data.id===props.chat.id?"bg-green-400":"bg-blue-400";
    return (
        <div className={`flex gap-2 items-center text-left text-white ${side}`}>
            ...
            <div className={`${colour} rounded-3xl w-auto p-2 relative flex items-center h-6 text-ss`}>
                {!(user.data.id===props.chat.id) && <span className="text-sxs absolute -top-4 left-0">
                    {props.chat.name+':'}
                </span>}
